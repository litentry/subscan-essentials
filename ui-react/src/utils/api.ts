...
export const useBlock = (host: string, data: getBlockParams | null) => {
    return useSWR<APIWrapperProps<blockType>, Error>(data ? [host, '/api/scan/block', data] : null, runtimeFetcher);
};
...
export const usePVMBlock = (host: string, data: getPVMBlockParams | null) => {
    return useSWR<APIWrapperProps<pvmBlockInfoType>, Error>(data ? [host, '/api/plugin/evm/block', data] : null, runtimeFetcher);
};
